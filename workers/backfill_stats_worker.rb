...
class BackfillStatsWorker
  ...
  def perform(hash)
    ...
    github_repo.github_issues_dataset.filter(state: 'open').each do |issue|
      data = generate_issue_stats(github_repo, issue)
      id = IssueStat.insert_conflict(
        target: [:github_repo_id, :issue_number],
        update: data
      ).insert(data)
    end
  ...
  def generate_issue_stats(github_repo, issue)
    ...
    # Get comment stats
    comment_users_stats, number_of_core_contributors_commented = fetch_comments_stats(issue)
  ...
  def fetch_comments_stats(issue)
    comments = issue.github_comments
  
    users_stats = {}
  
    comments.each do |comment|
      user_login = comment.author_login
